...
def update_parameters(x):
    # 取得滑動條的數值
    canny_threshold1 = cv2.getTrackbarPos('Threshold 1', 'Adjust Contours and Edges')
    canny_threshold2 = cv2.getTrackbarPos('Threshold 2', 'Adjust Contours and Edges')
    ...
    # 進行二值化處理
    _, edges = cv2.threshold(gray, canny_threshold1, canny_threshold2, cv2.THRESH_BINARY)
...
# 讀取圖片
original_img = cv2.imread('data/1.jpg')

# 建立視窗
cv2.namedWindow('Adjust Contours and Edges')

# 建立滑動條
cv2.createTrackbar('Threshold 1', 'Adjust Contours and Edges', 127, 255, update_parameters)
cv2.createTrackbar('Threshold 2', 'Adjust Contours and Edges', 255, 255, update_parameters)
